...
GlobalText = "FPS:" + Math.floor(1000 / 60) + " XFRICTION: " + ball.yfriction + " YFRICTION: " + ball.xfriction + " XSPEED: " + ball.vx + " YSPEED: " + ball.vy
...
rectArray = [];
text = new Objects.Text("DEFAULT", font_size + "px bolder header", GCENTER_X, GCENTER_Y, "white");
textArray = [];
...
function createBalls(x, y) {
  if (ballsNumber != 0 || ballsNumber != undefined || ballsNumber != null) {
    for (var num = 0; num <= BALL_NUMBERS; num++) {
      ...
      var balls = new Objects.Ball(xrandom, yrandom, randomRadius, randomCol);

      balls.vx = random(-3, 3);
      balls.vy = random(2, 5);
      ball.xfriction = random(0.20, 0.5);
      ball.yfriction = random(0.20, 0.5);
    ...
    GlobalText = "undefined number please resfresh the page and try it again";
  }
}
